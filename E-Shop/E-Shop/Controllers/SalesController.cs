        ...
        public ActionResult Buy2(int id)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    db.Sales.Add(satis);
                    db.SaveChanges();
                    ViewBag.islem = "Satın Alma İşlemi Başarıyla Gerçekleşmiştir";
                }
            }
            catch (Exception)
            {

                ViewBag.islem = "Satın Alma İşlemi Başarısız Gerçekleşmiştir";
            }
            return View("islem");
        }
        public ActionResult Buyall(decimal? tutar)
        {
            if(User.Identity.IsAuthenticated)
            {
                ...
                if(model!=null)
                {
                    if (kid == null)
                    {
                        ViewBag.tutar = "Sepetenizde Ürün Bulunmamaktadır";
                    }
                    else if(kid!=null)
                    {
